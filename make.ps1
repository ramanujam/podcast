...
#>
function lint() {
    poetry run flake8 src\podcasts tests
}
...
#>
function coverage() {
    poetry run coverage run --source src\podcasts -m pytest
...
#>
function docs(){
    if (Test-Path ".\docs\podcasts.rst") { Remove-Item ".\docs\podcasts.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ src\podcasts
